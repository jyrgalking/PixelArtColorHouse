...
public class Board : MonoBehaviour
{
    ...
    private Vector3 boardCamOriginalPosition;
    ...
    private void LoadUncompleteGame()
    {
        ...
        for (int i = 0; i < size; i++)
        {
            for (int j = 0; j < size; j++)
            {
                ...
                if (color.a != 0 && color != Color.white)
                {
                    ...
                    Tile tile = tilePooler.GetPooledTile();
        ...
        float freeHeight = mainCanvasTr.rect.height - bottomHeight - 120;
        ...
        boardCamOriginalPosition = new Vector3(0, -0.5f * boardSize + maxOrthographicSize * camCoef, boardCam.transform.position.z);
        boardCam.transform.position = boardCamOriginalPosition;

        if (Application.isMobilePlatform)
        {
            leanCameraZoom.enabled = false;
        }
        else
        {
            leanCameraZoom.enabled = true;
            leanCameraZoom.Zoom = maxOrthographicSize;
            leanCameraZoom.ZoomMin = minOrthographicSize;
            leanCameraZoom.ZoomMax = maxOrthographicSize;
        }
    ...
    private BottomInfo GetBottomInfo()
    {
        ...
        info.column = (int)Mathf.Ceil(info.width / 110f);
    ...
    private void LoadCompleteGame()
    {
        ...
        float width = bottomInfo.width, cellSize = bottomInfo.cellSize;
        float bottomHeight = cellSize * bottomInfo.row;
        float freeHeight = mainCanvasTr.rect.height - bottomHeight;
    ...
    public void PaintTile(Tile tile)
    {
        ...
        if (GetSelectedNumber() == -1)
        {
            ...
            leanCameraZoom.ZoomMax = maxOrthographicSize * 2;

            iTween.ValueTo(gameObject, iTween.Hash("from", boardCam.orthographicSize, "to", (maxOrthographicSize*2), "time", 0.3f, "onupdate", "OnUpdateValue"));
            iTween.MoveTo(boardCam.gameObject, boardCamOriginalPosition, 0.3f);
